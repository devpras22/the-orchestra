...
import { AgentSet, AgentData, DEFAULT_AGENT_SET_ID, getAgentSet } from '../data/agents'

// Omit 'id' and 'agents' when creating - those are generated
export type NewTeamData = Omit<AgentSet, 'id' | 'agents'>;
...
interface AgencyState {
  ...
  customAgents: Record<string, AgentData[]>;

  // ── Custom Teams (persisted) ─────────────────────────────────
  customTeams: Record<string, AgentSet>;
  ...
  addCustomAgent: (agentSetId: string, agent: Omit<AgentData, 'index'>) => void;
  updateCustomAgent: (agentSetId: string, agentIndex: number, agent: Omit<AgentData, 'index'>) => void;
  removeCustomAgent: (agentSetId: string, agentIndex: number) => void;

  // ── Actions — Custom Teams ───────────────────────────────────
  addCustomTeam: (team: AgentSet) => void;
  updateCustomTeam: (teamId: string, updates: Partial<AgentSet>) => void;
  deleteCustomTeam: (teamId: string) => void;
...
export const useAgencyStore = create<AgencyState>()(
  persist(
    (set) => ({
      ...
      addCustomAgent: (agentSetId, agent) => set((s) => {
        const existing = s.customAgents[agentSetId] || [];
        return {
          customAgents: {
            ...s.customAgents,
            [agentSetId]: [...existing, { ...agent, index: 0 }], // index will be assigned by getAgentSet
          },
        };
      }),

      updateCustomAgent: (agentSetId, agentIndex, agent) => set((s) => {
        const existing = s.customAgents[agentSetId] || [];
        return {
          customAgents: {
            ...s.customAgents,
            [agentSetId]: existing.map((a, i) => i === agentIndex ? { ...agent, index: 0 } : a),
          },
        };
      }),

      removeCustomAgent: (agentSetId, agentIndex) => set((s) => {
        const existing = s.customAgents[agentSetId] || [];
        return {
          customAgents: {
            ...s.customAgents,
            [agentSetId]: existing.filter((_, i) => i !== agentIndex),
          },
        };
      }),

      // ── Custom Team Actions ─────────────────────────────────────
      addCustomTeam: (team) => set((s) => ({
        customTeams: {
          ...s.customTeams,
          [team.id]: team,
        },
      ...
      updateCustomTeam: (teamId, updates) => set((s) => {
        const existing = s.customTeams[teamId];
        if (!existing) return {};
        return {
          customTeams: {
            ...s.customTeams,
      ...
      deleteCustomTeam: (teamId) => set((s) => {
        // Remove the team and its custom agents
        const { [teamId]: _, ...remainingTeams } = s.customTeams;
        const { [teamId]: __, ...remainingAgents } = s.customAgents;

        // If deleting the currently selected team, switch to default
        const newSelectedId = s.selectedAgentSetId === teamId
          ? DEFAULT_AGENT_SET_ID
          : s.selectedAgentSetId;

        return {
          customTeams: remainingTeams,
          customAgents: remainingAgents,
          selectedAgentSetId: newSelectedId,
          // Reset project state if we switched teams
          ...(s.selectedAgentSetId === teamId ? {
            clientBrief: '',
            phase: 'idle',
            finalOutput: null,
            tasks: [],
...
export function getActiveAgentSet(): AgentSet {
  const { selectedAgentSetId, customAgents, customTeams } = useAgencyStore.getState()
  return getAgentSet(selectedAgentSetId, customAgents, customTeams)
}
