...
enum HookInstaller {
    ...
    private static let hookScriptPath = NSHomeDirectory() + "/.orchestra-desktop/hooks/hook-sender.sh"
    private static let hookCommand = "~/.orchestra-desktop/hooks/hook-sender.sh"
    ...
    static func ensureScriptExists() throws {
        ...
        // Write script - Forwards Claude Code hook events to The Orchestra
        let script = """
        #!/bin/bash
        \(scriptVersion)
        # hook-sender.sh — Forwards Claude Code hook events to The Orchestra
        ...
        if [ "$EVENT_NAME" = "PermissionRequest" ]; then
            ...
            TMPFILE=$(mktemp /tmp/orchestra-hook.XXXXXX)
            curl -s -w "\\n%{http_code}" -X POST \\
